...
export async function POST(req: Request) {
  ...
  const body = (await req.json()) as CreateCsmProjectForm;
  const csmProjectService = await CsmProjectService();
  const { error } = await csmProjectService.createProject(user.email, body);
